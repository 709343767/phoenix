...
public class MethodExecuteHandler {
    ...
    public static BaseResponsePackage sendHeartbeatPackage2Server(HeartbeatPackage heartbeatPackage) {
        ...
        Invoker invoker = InvokerHolder.getInvoker(com.imby.agent.business.server.service.IHeartbeatService.class,
    ...
    public static BaseResponsePackage sendAlarmPackage2Server(AlarmPackage alarmPackage) {
        // 通过命令执行器管理器，获取指定的命令执行器
        Invoker invoker = InvokerHolder.getInvoker(com.imby.agent.business.server.service.IAlarmService.class,
                "sendAlarmPackage");
        // 执行命令，返回执行结果
    ...
    public static BaseResponsePackage sendServerPackage2Server(ServerPackage serverPackage) {
        // 通过命令执行器管理器，获取指定的命令执行器
        Invoker invoker = InvokerHolder.getInvoker(com.imby.agent.business.server.service.IServerService.class,
                "sendServerPackage");
        // 执行命令，返回执行结果
