...
## 结论
Phoenix数据库设计围绕“监控服务器、实例、协议（HTTP/网络/TCP）、JVM指标、告警”五大维度构建，辅以环境/分组/用户/角色的组织与权限体系。通过合理的索引与约束、MyBatis-Plus配置与Druid连接池，满足高并发下的稳定性与可维护性。建议在生产环境中配合历史数据分区与定期清理策略，持续优化查询与写入性能。
...
- 使用建议
  - 服务端：通过实体类与Mapper接口访问MONITOR_SERVER、MONITOR_INSTANCE、MONITOR_DB等表
  - UI端：通过实体类与Mapper接口访问MONITOR_DB等表，结合代码生成器快速生成DAO与XML
