...
public class MonitorNetServiceImpl extends ServiceImpl<IMonitorNetDao, MonitorNet> implements IMonitorNetService {
    ...
    public Page<MonitorNetVo> getMonitorNetList(Long current, Long size, String ipSource, String ipTarget, String status,
                                                String monitorEnv, String monitorGroup, String ipDesc, String isEnableMonitor,
        ...
        if (StringUtils.isNotBlank(ipDesc)) {
            lambdaQueryWrapper.like(MonitorNet::getIpDesc, ipDesc);
        }
        if (StringUtils.isNotBlank(isEnableMonitor)) {
            lambdaQueryWrapper.eq(MonitorNet::getIsEnableMonitor, isEnableMonitor);
        }
        if (StringUtils.isNotBlank(isEnableAlarm)) {
            lambdaQueryWrapper.eq(MonitorNet::getIsEnableAlarm, isEnableAlarm);
        }
