...
import com.gitee.pifeng.server.util.EndpointUtils;
...
/**
 * <p>
 * 在项目启动后，定时扫描应用实例池中的所有应用实例，更新应用实例状态（应用实例池、数据库），发送告警。
 * </p>
 *
 * @author 皮锋
...
public class InstanceMonitorTask implements CommandLineRunner {
    ...
    private void sendAlarmInfo(String title, AlarmLevelEnums alarmLevelEnums, Instance instance) throws NetException, SigarException {
        String msg = "应用ID：" + instance.getInstanceId()
                ...
                + "，<br>应用端点：" + EndpointUtils.instanceEndpointNameEn2Cn(instance.getInstanceEndpoint())
