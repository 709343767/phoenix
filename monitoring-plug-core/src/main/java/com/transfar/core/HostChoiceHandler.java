...
public class HostChoiceHandler {
    ...
    public static Host choiceHost() {
        ...
        if (StringUtils.isNotBlank(serverUrl)) {
            // 主机为服务端
            return Host.builder().url(serverUrl + SERVER_NAME).username(serverUserName)
                    .password(serverPassword).build();
        } else {
            // 主机为代理端
            return Host.builder().url(agentUrl + AGENT_NAME).username(agentUserName).password(agentPassword)
                    .build();
        }
    }
