...
public class DesEncryptUtils extends InitSecurer {
    ...
    public static String encrypt(String str) {
        ...
        try {
            ...
            // 取MD5Hash码，并组合加密数组
            byte[] md5Hasn = md5Hash(encrypt, 0, encrypt.length);
            // 组合消息体
            ...
            byte[] temp = desCbcEncrypt(totalByte, deskey, ivParam);
    ...
    public static String decrypt(String str) {
        ...
        try {
            ...
            SecretKeySpec deskey = new SecretKeySpec(key, ALGORITHM);
            IvParameterSpec ivParam = new IvParameterSpec(iv);
            byte[] temp = desCbcDecrypt(encBuf, deskey, ivParam);
            // 进行解密后的md5Hash校验
    ...
    private static void getkeyiv(byte[] key, byte[] iv) {
        // 密钥Base64解密
        byte[] buf = Base64.getDecoder().decode(KEY);
        // 前8位为key
        int i;
